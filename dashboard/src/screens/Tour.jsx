...
function S6() {
  const rows = [
    ['Annual room revenue (12 rms · $350 · 65%)', '$99,540', 'white'],
    ['Revenue lift @ 15% (conservative)', '+$14,931', 'emerald'],
    ['Average monthly lift', '+$1,244', 'emerald'],
    ['Professional subscription', '−$699', 'rose'],
    ['Net monthly benefit', '+$545', 'emerald'],
  ]
  const tone = { white: 'text-white', emerald: 'text-emerald-300', rose: 'text-rose-300' }
  return (
    <div className="grid grid-cols-3 gap-4 items-center">
      <div className="text-center rounded-xl bg-navy-dark/60 border border-gold/40 p-5">
        <div className="text-gold-light text-xs uppercase">Return on subscription</div>
        <div className="text-5xl font-extrabold text-gold mt-1">1.8×</div>
        <div className="text-white/50 text-[11px] mt-1">conservative · climbs past 2× at 20%</div>
      </div>
      <div className="col-span-2 space-y-1.5 text-sm">
        {rows.map((r) => (
          <div key={r[0]} className="flex items-center justify-between rounded-lg bg-white/5 px-3 py-2">
            <span className="text-white/60 text-xs">{r[0]}</span>
            <span className={`font-bold ${tone[r[2]]}`}>{r[1]}</span>
          </div>
        ))}
      </div>
    </div>
...
const STEPS = [
  ...
  { id: 'step_06', title: 'ROI Performance', duration: 90, Sim: S6, narration: N.s6, callouts: [['Subscription', '$699/month'], ['Avg Monthly Lift', '$1,244'], ['Net Monthly Benefit', '$545'], ['Return', '1.8× your subscription']] },
...
function Closing({ onRestart, muted, onMute }) {
  const stats = [['1.8×', 'return on subscription (15% lift)'], ['$1,244', 'avg monthly revenue lift'], ['$28,662', 'private event identified'], ['$840', 'gap night recovery this week']]
  const tiers = [
