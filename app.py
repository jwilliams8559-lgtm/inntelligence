...
DEMO_NARRATION = {
    ...
    "step_07": "Beyond weddings, the Private Events screen handles corporate retreats, family reunions, full buyouts, and group bookings, all with the same intelligence. Conflict detection tells Sarah whether to take a group on any given date, and the exclusivity calculator makes sure she never under-prices a buyout. Then there are the Packages, and this is where Inn-telligence really pulls ahead of every other pricing tool. We have built in the twenty-five packages boutique inns reach for most often: romance, anniversary, culinary, birding and nature, spa, and more. Turn one on with a single click. Turn it off just as easily. Each one is pre-priced from market data and tied straight into the rate calendar, so the engine can measure true revenue per available room, not just the nightly rate. And the Gift Shop tracks retail right alongside lodging, so Sarah sees every dollar the property earns, from the rooms to the rooftop to the front-desk shelves.",
