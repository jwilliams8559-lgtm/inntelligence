...
_TOUR_STEP_IDS  = (
    ["opening"] + [f"step_{i:02d}" for i in range(1, 15)] + ["closing"]
    + [f"demo_{i:02d}" for i in range(1, 7)] + ["demo_closing"]
)
...
_TOUR_NARRATION = {
    ...
    # ── Interactive /demo narration — short, punchy, synced to the real screens
    "demo_01": "This is the real Inn-telligence dashboard for The Bay Street Inn, a nineteen-room boutique inn in Beaufort, South Carolina. You are looking at the live ninety-day Rate Calendar. Every cell is an AI-generated rate recommendation for one room on one night. Notice the alert at the top: the Beaufort Water Festival is driving peak demand this July. Inn-telligence detected it automatically and began pricing for it months ahead.",
    "demo_02": "Click any date, and Inn-telligence shows you exactly why it recommends that rate. Demand score. Competitor rates. Seasonal index. Lead time. And a plain-English explanation. This is not a black box. Every recommendation is transparent, and you can accept it or override it in a single click.",
    "demo_03": "Inn-telligence monitors your Beaufort competitors around the clock and compares you like-for-like, by room type. Here in the waterfront category, Cuthbert House Inn is one of your closest direct competitors. Inn-telligence never compares you to a property that is not truly competing for the same guest — and that nuance is exactly what protects your premium.",
    "demo_04": "This is your demand forecast. Inn-telligence reads the local Beaufort events calendar and translates it into revenue. Every event is scored by its pricing impact, and the biggest demand drivers are surfaced automatically — each with the exact recommended action, months ahead of the date.",
    "demo_05": "Inn-telligence builds a complete profile on every guest who has ever stayed with you, and segments them automatically — VIPs, regulars, and lapsed guests worth winning back. When your occupancy forecast dips, it can trigger a personalized win-back campaign on its own. The best revenue you will ever earn comes from guests who already love you.",
    "demo_06": "And here is the moment that changes your morning. When you are ready, approve every recommendation in one click — and Inn-telligence publishes your rates simultaneously to Booking.com, Expedia, Airbnb, V-R-B-O, Hotels.com, Trip.com, and Agoda. What used to take hours, updating every channel by hand, now takes one click.",
    "demo_closing": "That is Inn-telligence — real pricing intelligence for boutique inns, built by a revenue professional who now owns one himself. It starts at three hundred and ninety-nine dollars a month, with a limited number of Founding Member slots available free for six months. Explore the dashboard yourself, or request your Founding Member access today.",
}
