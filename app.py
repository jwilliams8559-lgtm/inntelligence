...
def api_room_rate():
    ...
    # Demand score (0–100) for the drawer gauge — a composite of event,
    # seasonal, and demand multipliers, with priority-ordered drivers.
    try:
        ev = float(detail.get("event_multiplier") or 1.0)
        si = float(detail.get("seasonal_index") or 1.0)
        ed = max(float(detail.get("effective_demand") or 1.0), 1.0)
        score = max(0, min(100, round((ev * si * ed - 0.9) * 85)))
        if   score >= 91: label = "Peak"
        elif score >= 76: label = "Very High"
        elif score >= 61: label = "High"
        elif score >= 41: label = "Normal"
        elif score >= 16: label = "Low"
        else:             label = "Very Low"
        drivers = []
        for e in (detail.get("active_events") or []):
            drivers.append(str(e))
        if si >= 1.08:
            drivers.append("Peak season pricing")
        try:
            occ = float(detail.get("occupancy_rate") or 0)
            if occ >= 0.7:
                drivers.append("High occupancy / limited availability")
        except (TypeError, ValueError):
            pass
        detail["demand_score"] = score
        detail["demand_label"] = label
        detail["demand_drivers"] = drivers[:3]
    except (TypeError, ValueError):
        detail["demand_score"] = None
...
# ─────────────────────────────────────────────────────────────────────────────
#  Interactive /demo narration — 11-step guided tour over the real dashboard,
#  framed by an opening (founder intro) and a closing (CTA). Same ElevenLabs
#  "Will" voice as the legacy tour; cached MP3s under public/audio/demo.
# ─────────────────────────────────────────────────────────────────────────────
_DEMO_AUDIO_DIR = os.path.join(_BASE_DIR, "dashboard", "public", "audio", "demo")
_DEMO_STEP_IDS  = [f"step_{i:02d}" for i in range(0, 12)] + ["closing"]

DEMO_NARRATION = {
    "step_00": "My name is Jim Williams. For eleven years I built enterprise pricing systems for one of America's largest companies — using artificial intelligence to optimize hundreds of millions of dollars in revenue. I managed pricing teams, built machine learning models, and learned everything there is to know about how data-driven pricing creates competitive advantage. Then I fell in love with the Lowcountry. With Byoo-furt, South Carolina. With the historic inns on Bay Street, with the Spanish moss and the Byoo-furt River, with the idea that independent inn owners deserve the same pricing intelligence that Fortune 500 companies use every day. Here is something I want to be clear about from the start. Artificial intelligence is powerful — it sees patterns no human can see and processes data at speeds no team can match. But AI alone is not enough. The best revenue decisions combine AI intelligence with human judgment. INNtelligence is built on this philosophy. The engine analyzes your market, monitors your competitors, forecasts your demand, and generates recommendations. But every recommendation waits for your review and your approval before anything changes. You are always in control. INNtelligence gives you the intelligence. You make the call. INNtelligence connects directly to the property management systems your inn already uses — ResNexus, Cloudbeds, ThinkReservations, Guesty, WebRezPro, and Little Hotelier. And it publishes approved rates simultaneously to Booking dot com, Expedia, Airbnb, VRBO, Hotels dot com, Trip dot com, and Agoda. No manual re-entry. No copy and paste. One approval in INNtelligence and your rates are live everywhere your guests are booking. INNtelligence is available for properties across the United States, Canada, the United Kingdom, Europe, and the Caribbean. When TakeUp AI closed their doors on May first — twenty-five days ago — their customers had approximately thirty days notice to download their data and figure out what came next. I built INNtelligence because independent inn owners deserve a solution that combines the power of artificial intelligence with the irreplaceable judgment of the innkeeper who knows their property and their guests better than any algorithm ever will. This is INNtelligence. Let me show you what it does for The Bay Street Inn in Byoo-furt, South Carolina.",
    "step_01": "It is Tuesday morning at The Bay Street Inn in downtown Byoo-furt. The inn sits on the waterfront — nineteen beautifully appointed rooms, The Parlor at Bay Street restaurant serving dinner nightly to inn guests and Byoo-furt locals, and a rooftop bar that has become one of the most sought-after spots in downtown Byoo-furt on weekend evenings. Sarah, the owner, is having her first cup of coffee. She picks up her iPad. INNtelligence is designed mobile-first. Every single feature you are about to see works identically on an iPhone, an Android phone, an iPad, or any tablet. Sarah runs her entire revenue strategy from a screen that fits in her pocket. No laptop required. No office required. From the breakfast table, the front porch, the farmers market, or poolside — INNtelligence goes wherever Sarah goes. The days of being chained to a desktop to manage your rates are over. INNtelligence has been running all night. It monitored every lodging property within twenty-five miles of Bay Street Inn — thirty-nine properties in the Byoo-furt market, from the Montage Palmetto Bluff to the Hampton Inn. It checked her booking pace against the same period last year. It cross-referenced the local event calendar against five years of historical occupancy patterns. And it has something important to tell her. The Byoo-furt Water Festival begins in fifty-two days. Peak demand detected. Every recommendation is waiting for Sarah's review. Nothing has changed yet. Nothing ever changes until Sarah says so.",
    "step_02": "The Rate Calendar is the heart of INNtelligence. Ninety days of AI-generated rate recommendations — every room type, every night, color coded by opportunity. The time period selector at the top lets Sarah view seven days, two weeks, one month, or the full ninety-day quarter. Right now she is looking at the full quarter so she can see everything coming. The colors tell the story instantly. Navy cells are rate increase recommendations — the deeper the navy, the stronger the opportunity. Red cells are rate decrease recommendations for slower periods where a modest adjustment drives occupancy. And those gold columns — July seventeenth through the twenty-sixth — that is the Water Festival. INNtelligence has assigned those dates a demand score of ninety out of one hundred. Peak classification. The highest tier in the system. But these are recommendations only. Nothing has changed. Nothing goes live until Sarah reviews each recommendation and approves. The AI did the analysis. Sarah makes the call.",
    "step_03": "Sarah taps a Water Festival night on her Waterfront Suite row. The recommendation drawer opens. This is where INNtelligence shows its work. The recommended rate is a significant premium over the suite's normal nightly rate — but Sarah does not just see a number. She sees exactly why the engine arrived at it, and she can agree, adjust, or override based on her own knowledge of her property and her market. The demand score reads ninety out of one hundred. Three demand drivers in priority order. Water Festival — the peak annual event, historically the highest occupancy period for every boutique inn in Byoo-furt. Competitor compression — primary waterfront competitors filling fast, with sold-out status appearing on key dates. And booking pace running forty-seven percent ahead of last July at this same lead time. Now look at the OTA commission analysis. After Booking dot com's fifteen percent commission, a direct booking through Bay Street Inn's own website nets more — and the guest still saves money. Everyone wins. The guest is rewarded. Sarah keeps more revenue. And every direct booking captures the guest's email for future marketing, something an OTA booking never gives her. A three-night minimum is recommended for Water Festival dates, because a two-night booking on a peak weekend blocks a three-night booking from filling that room. INNtelligence catches this automatically. Sarah reviews the reasoning, agrees with the logic, and approves.",
    "step_04": "Before approving her full rate set Sarah checks Competitive Intelligence. INNtelligence discovered thirty-nine lodging properties within twenty-five miles of Bay Street Inn. It ranked all thirty-nine using a five-dimension scoring system — price band overlap, amenity similarity, distance, review tier, and property size — to identify the nine competitors that most directly affect Bay Street Inn's pricing. But here is a feature that makes INNtelligence unique in how it handles competitive analysis. Sarah is reviewing the Waterfront Suite. She selects Waterfront Suite from the room type filter. Watch what happens. Competitors without waterfront rooms gray out immediately. Rhett House — a beautiful historic inn but not waterfront — grays out. The upscale hotels without waterfront inventory gray out. What remains are only the competitors whose waterfront rooms actually compete with Bay Street Inn's Waterfront Suite. Comparing a waterfront suite to a garden view room at a competitor is comparing apples to oranges. INNtelligence compares the right products to each other every time. Among the active waterfront competitors — Cuthbert House Inn scores ninety-two point seven. Waterfront, three tenths of a mile away, four-point-nine stars, ten rooms. And the Anchorage 1770 Inn scores eighty-eight point five. Also waterfront, fifteen rooms, four-point-eight stars. Both are selling out on Water Festival dates. Cuthbert at five sixty — sold out. Anchorage at five thirty-five — limited availability. When both primary waterfront competitors are at capacity, Bay Street Inn becomes the waterfront option in Byoo-furt. INNtelligence detected this compression and built it into the five-ninety-nine recommendation. The price ladder tells the complete story. Montage Palmetto Bluff at eleven hundred dollars is the luxury ceiling. Cuthbert and Anchorage are the boutique waterfront benchmark. Bay Street Inn at five ninety-nine is correctly positioned. Hampton Inn at one ninety-five is the budget floor. Five ninety-nine is not just a number. It is a defensible market position.",
    "step_05": "Sarah clicks Events and selects the year view. Every demand driver INNtelligence tracks for the Byoo-furt market — organized by time period so Sarah can plan at whatever horizon matters. She can view this week, this month, this quarter, or the full year ahead. This week — MCRD Parris Island graduation on Friday. Marines commissioned every Friday at Parris Island, families traveling from across the country, boutique inns in Byoo-furt filling reliably every week. INNtelligence applies a demand boost automatically every Friday throughout the year because the data consistently shows it. The Gullah Festival in late May — one of America's most important celebrations of Gullah Geechee culture, right here in Byoo-furt. Water Festival in July — peak annual demand. The Byoo-furt Film Festival in fall. The winter holidays. New Year's Eve. Looking at the full year Sarah sees every major demand driver plotted against her current booking pace — twelve months of revenue intelligence with time to prepare for every peak period. No surprises. No scrambling. The engine tracks it all so Sarah never has to.",
    "step_06": "A new wedding inquiry just arrived. Margaret and Thomas are considering Bay Street Inn for their September wedding. Forty guests. Full property buyout for two nights. Before INNtelligence, Sarah would spend thirty minutes with a spreadsheet figuring out whether to accept and what to charge. Often guessing. Sometimes guessing wrong. INNtelligence calculates the complete package instantly. Room block for all nineteen rooms for two nights at September rates. Event space rental. Food and beverage package. Setup and breakdown. And the exclusivity premium — the price of turning away every other guest for those two nights, calculated from projected displaced revenue. Total package: thirty-seven thousand, three hundred dollars. INNtelligence compares this to individual booking revenue for the same September weekend. The wedding premium above regular revenue — over twenty thousand dollars. A clear accept. But now watch what happens when Sarah considers a Water Festival date. The conflict alert fires immediately. Water Festival individual pricing exceeds the wedding buyout value on those specific dates. INNtelligence recommendation — decline or negotiate above festival pricing. That one calculation, on that one date change, just protected thousands of dollars Sarah would never have caught without INNtelligence. The AI flagged it. Sarah decides.",
    "step_07": "Beyond weddings, the Private Events screen manages corporate retreats, family reunions, buyouts, and group bookings — all with the same revenue intelligence. Conflict detection tells you whether to accept a group on any given date. The exclusivity premium calculator ensures you never under-price a buyout. The Packages screen is where INNtelligence really separates itself from every other pricing tool on the market. We have built in the twenty-five most popular packages that boutique inns across the country offer — romance packages, anniversary experiences, Lowcountry culinary tours, birding and nature packages, spa packages, and more. Turn any package on with one click. Turn it off just as easily. Every package is pre-priced based on market data and integrated directly with your rate calendar — so INNtelligence can calculate true total revenue per available room, not just the nightly rate. The Gift Shop tracks retail revenue alongside room revenue, giving you a complete picture of every dollar your property generates.",
    "step_08": "Sarah clicks Guest CRM. Every guest automatically segmented every night by INNtelligence. Seven VIP guests — three or more stays or fifteen hundred dollars in lifetime revenue. Eight local guests from South Carolina and Georgia who drive in for weekend escapes. Three lapsed guests who have not returned in over a year. Four new guests from their first stay in the last ninety days. Two anniversary guests identified from booking notes and stay history. Because INNtelligence is forecasting occupancy below fifty-five percent for two weeks in November, it has automatically drafted an email campaign targeting the lapsed guest segment with a special return rate and a personal message. The campaign was written by the AI based on Bay Street Inn's voice and guest history. Sarah reads it. She knows one of the lapsed guests personally and adds a specific handwritten touch. She approves. INNtelligence sends. The AI created the opportunity and wrote the first draft. The human made it personal. That combination is what drives results. Every guest who connects to the wifi at Bay Street Inn is prompted to share their email and consent to future communications — captured at the moment they are most engaged, sitting in a beautiful waterfront inn in Byoo-furt, feeling the magic of the Lowcountry. That email becomes a relationship. That relationship becomes a repeat booking with no OTA commission and no acquisition cost.",
    "step_09": "Sarah checks Reputation. INNtelligence aggregates reviews from Google, TripAdvisor, and Booking dot com into a single feed, scores sentiment automatically, and flags anything requiring a human response. Bay Street Inn carries a four-point-eight overall reputation score across all platforms. The trend is up from four-point-six twelve months ago — reflecting the improved guest experience that comes from consistent pricing, reduced overbooking pressure, and the operational focus that comes from spending less time on manual rate management. One review is flagged. A guest mentioned check-in felt slightly rushed during a busy arrival afternoon. INNtelligence flagged it with a suggested response. Sarah personalizes it in her own voice and posts directly from the platform. Reputation is revenue. Research consistently shows that a move from four-point-six to four-point-eight on TripAdvisor correlates with a six to eight percent increase in booking conversion. INNtelligence tracks this connection so Sarah can see the revenue impact of every guest experience improvement.",
    "step_10": "Sarah checks her performance numbers. This month versus last year — occupancy up nine percent, average daily rate up sixteen percent, RevPAR up twenty-six percent. Not accidental. The result of ninety days of INNtelligence recommendations reviewed, validated, and approved consistently. The AI provided the recommendations. Sarah provided the judgment. Together they produced a twenty-six percent RevPAR improvement. Her Professional subscription is six ninety-nine per month. But the return is not just from room rates. INNtelligence lifts revenue across every part of the property. Room rate optimization adds sixty thousand dollars annually at a fifteen percent lift. Shifting just ten percent of bookings from OTA to direct saves seven thousand dollars in commissions. Guest CRM campaigns bring back lapsed guests, adding seventy-five hundred in repeat revenue. Package and upsell optimization adds four thousand. Gift shop and F and B improvements add fifteen thousand combined. Total annual lift across all revenue streams — ninety-three thousand, five hundred dollars. Annual subscription cost — eight thousand, three hundred and eighty-eight. Total annual return on your INNtelligence investment — eleven-point-one times your subscription cost. Not room revenue alone. Eleven times your total investment across every revenue stream your property generates. That is the real number.",
    "step_11": "Sarah has spent fifty-five minutes with her morning coffee. She has reviewed every Water Festival recommendation and understood the reasoning behind each one. Confirmed her competitive position against Cuthbert and the Anchorage — and confirmed her five-ninety-nine rate is defensible. Accepted a September wedding on terms that work for the inn. Approved a guest campaign she personalized with her own touch. Responded to a flagged review in her own voice. Confirmed her RevPAR is up twenty-six percent. Fifty-five minutes. Every major revenue decision for Bay Street Inn reviewed, considered, and acted upon. Now she taps Approve All. In two-point-two-six seconds, INNtelligence publishes updated rates to Booking dot com, Expedia, Airbnb, VRBO, Hotels dot com, Trip dot com, and Agoda simultaneously. Every OTA. Every room type. Every date in the recommendation window. The rates are live everywhere. Sarah closes her iPad and walks downstairs to greet her guests at breakfast. Before INNtelligence she spent two hours every Monday doing this manually — often wrong, always leaving money on the table, never having the competitive intelligence to know what her market was actually doing. Now she spends fifty-five minutes once a week, reviews every recommendation with full context and reasoning, makes her own judgment calls, and publishes with confidence. The AI works all night so the innkeeper can work smarter in the morning.",
    "closing": "INNtelligence is built exclusively for independent boutique inns and Select Registry properties. Not for hotel chains. Not for vacation rental platforms. For the kind of property where the owner knows every guest by name, where the history of the building is part of the experience, where excellence is deeply personal. INNtelligence combines artificial intelligence with human judgment — because the best revenue decisions are made when powerful data meets the irreplaceable knowledge of the innkeeper who lives and breathes their property every day. We are selecting three to five founding members for our pilot program. Founding members receive the complete Professional tier of INNtelligence — everything you just saw in this demonstration — free for six months. In exchange we ask for three things. Connect your property management system. Participate in monthly feedback calls. And share your honest testimonial after ninety days if the results warrant it. After your founding period — six hundred and ninety-nine dollars per month. And the return is across your entire property, not room rates alone. Room optimization, direct booking commission savings, guest CRM campaigns, package and upsell revenue, gift shop, and food and beverage — together, a total annual lift of ninety-three thousand, five hundred dollars against an annual subscription of eight thousand, three hundred and eighty-eight dollars. A total annual return on investment of eleven-point-one times. Conservative. Auditable. Real. INNtelligence is already built and integrated. ResNexus, Cloudbeds, ThinkReservations, Guesty, WebRezPro, and Little Hotelier — if your property management system has an open API, INNtelligence connects to it. Booking dot com, Expedia, Airbnb, VRBO, Hotels dot com, Trip dot com, and Agoda — your rates go live on all seven simultaneously with one click. No months of integration work. No technical team required. Connect your PMS in minutes, approve your first recommendations, and your rates are live everywhere your guests are searching — today. TakeUp AI closed their doors on May first — twenty-five days ago. Their customers had thirty days notice. INNtelligence is open, it is built, it is live, and it is looking for the right founding partners to shape what comes next in boutique hospitality revenue management. My name is Jim Williams. Four-seven-zero, seven-eight-nine, two-four-three-three. Click the button and I will personally respond within twenty-four hours.",
}


def _demo_path(step_id: str) -> str:
    return os.path.join(_DEMO_AUDIO_DIR, f"{step_id}.mp3")


def _demo_generate(step_id: str) -> tuple[str, bool]:
    """Return (path, cached). Generates the demo MP3 via ElevenLabs if missing."""
    path = _demo_path(step_id)
    if os.path.exists(path) and os.path.getsize(path) > 0:
        return path, True
    api_key = os.getenv("ELEVENLABS_API_KEY")
    if not api_key:
        raise RuntimeError("ELEVENLABS_API_KEY not configured")
    narration = DEMO_NARRATION.get(step_id)
    if not narration:
        raise RuntimeError(f"No narration text for {step_id}")
    import requests
    resp = requests.post(
    ...
    resp.raise_for_status()
    os.makedirs(_DEMO_AUDIO_DIR, exist_ok=True)
    with open(path, "wb") as f:
        f.write(resp.content)
    return path, False


@app.route("/api/demo/narration/<step_id>")
def api_demo_narration(step_id: str):
    """Return the narration text for a demo step (used for the speech fallback)."""
    if step_id not in _DEMO_STEP_IDS:
        return jsonify({"error": "Invalid step_id"}), 404
    return jsonify({"step_id": step_id, "text": DEMO_NARRATION.get(step_id, "")})


@app.route("/api/demo/generate-audio/<step_id>", methods=["POST"])
def api_demo_generate_audio(step_id: str):
    """Generate (or return cached) MP3 for one demo step."""
    if step_id not in _DEMO_STEP_IDS:
        return jsonify({"ok": False, "error": "Invalid step_id"}), 400
    try:
        _path, cached = _demo_generate(step_id)
        return jsonify({"ok": True, "step_id": step_id, "cached": cached,
                        "url": f"/api/demo/audio/{step_id}"})
    except Exception as e:  # noqa: BLE001 — surface to frontend for speech fallback
        logger.warning("Demo audio generation failed for %s: %s", step_id, e)
        return jsonify({"ok": False, "step_id": step_id, "error": str(e)}), 502


@app.route("/api/demo/audio/<step_id>")
def api_demo_audio(step_id: str):
    """Serve a demo step's MP3, generating on demand if missing."""
    if step_id not in _DEMO_STEP_IDS:
        return jsonify({"error": "Invalid step_id"}), 404
    path = _demo_path(step_id)
    if not (os.path.exists(path) and os.path.getsize(path) > 0):
        try:
            _demo_generate(step_id)
        except Exception as e:  # noqa: BLE001
            return jsonify({"error": str(e)}), 502
    return send_file(path, mimetype="audio/mpeg", conditional=True)
